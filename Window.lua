...
function Window:AddButton(options)
    return self.Elements:CreateButton(self.ScrollView, options)
end

function Window:AddSlider(options)
    return self.Elements:CreateSlider(self.ScrollView, options)
end

function Window:AddToggle(options)
    return self.Elements:CreateToggle(self.ScrollView, options)
end

function Window:AddLabel(options)
    return self.Elements:CreateLabel(self.ScrollView, options)
end

function Window:AddSeparator()
    return self.Elements:CreateSeparator(self.ScrollView)
end
...
function Window:UpdateLayout()
    ...
    for _, child in ipairs(self.ScrollView:GetChildren()) do
        if child:IsA("GuiObject") and child ~= self.Layout then
            canvasHeight = canvasHeight + child.AbsoluteSize.Y + 5
        end
    end
    self.ScrollView.CanvasSize = UDim2.new(0, 0, 0, canvasHeight)
end
