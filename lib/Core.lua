...
function UniversalGUI:CreateWindow(options)
    ...
    if not self.Window then
        error("Window module not loaded! Make sure Window.lua is loaded.")
        return nil
    end
...
function UniversalGUI:CreateFloatingButton(options)
    ...
    -- Dragging (se habilitado)
    if options.Draggable and self.Dragging then
        self.Dragging:EnableFloatingDrag(btn)
    end
